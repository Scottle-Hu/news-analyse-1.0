...
(function ($, window, document, undefined) {
    ...
    Plugin.prototype.navigateTo = function (path) {
        if (path=="/" || path.indexOf("default_to_home")>=0){
            path="/project"
        }
        var _parent = this;
        ...
        if(typeof choose1_option_val!="undefined" || typeof choose2_option_val!="undefined") {
            var cluster;
            if(_parent.options.inputElement.attr("id")=="val0"){
                    cluster=choose1_option_val.split(",")[0]
            }
            if(_parent.options.inputElement.attr("id")=="val1"){
                    cluster=choose2_option_val.split(",")[0]
            }

            $.getJSON("/filebrowser/chooser=" + path,{odeon_cluster:cluster}, function (data) {
            ...
            if (data.title != null && data.title == "Error") {
               ...
               /* if (_parent.options.showExtraHome) {
                ...
                if (_parent.options.createFolder) {
                   if(path.split("/").length>3 && (path.split("/")[3]==username_ || path.split("/")[3]==path.split("/")[2])){
            ...
            if (data.title != null && data.title == "Error") {
               ...
               $(data.files).each(function (cnt, file) {
                    filepath_arr=file.path.split("/")
                    if(filepath_arr.length<=3){
                         file_add(cnt, file)
                    }
                    else if(file.stats.user==username_ || filepath_arr[2]==filepath_arr[3]){
                      file_add(cnt, file)
                    }
