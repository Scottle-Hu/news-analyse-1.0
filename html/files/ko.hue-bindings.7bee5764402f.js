...
(function (factory) {
  ...
  function checkPosition ($dropDown, $select){
    var dropDownOffset = $dropDown[0].getClientRects()[0];
    var selectOffset = $select[0].getClientRects()[0];
    if (!dropDownOffset || !dropDownOffset) {
      return false;
    }
    console.log(dropDownOffset, selectOffset,  Math.abs(dropDownOffset.left - selectOffset.left), Math.abs(dropDownOffset.top - selectOffset.top));
    var isXMatch = Math.abs(dropDownOffset.left - selectOffset.left) < 10;
    var isYMatch = Math.abs(dropDownOffset.top - selectOffset.top) < 40 || Math.abs(dropDownOffset.bottom - selectOffset.bottom) < 40;
  ...
  ko.bindingHandlers.select2 = {
    init: function (element, valueAccessor, allBindingsAccessor, vm) {
      ...
      var $activeSelect2InBody = $('body > .select2-container');
      var isSelfVisible = $activeSelect2InBody.length ? checkPosition($activeSelect2InBody.find('.select2-dropdown'), $element) : false;
      ...
      $element
        ...
        if (isSelfVisible) {
          var searchKey = $activeSelect2InBody.find('input').val();
          $activeSelect2InBody.remove();
          $element.select2('open');
          if(searchKey.length) {
            $('body > .select2-container input').val(searchKey).trigger('input').focus();
          }
        }
    },
  ...
  function getFileBrowseButton(inputElement, selectFolder, valueAccessor, stripHdfsPrefix, allBindingsAccessor, isAddon, isNestedModal) {
    ...
    _btn.click(function (e) {
      ...
      function handleChoice(filePath, stripHdfsPrefix) {
        ...
        if (allBindingsAccessor && allBindingsAccessor().filechooserOptions && allBindingsAccessor().filechooserOptions.deploymentDir) {
           choose1_option_val=$("#choose1").find("option:selected").val()
        choose2_option_val=$("#choose2").find("option:selected").val()
        if(typeof choose1_option_val!=undefined || typeof choose2_option_val!=undefined){
          if(inputElement.attr("id")=="val0")
            filePath=choose1_option_val.split(",")[1] + filePath
            inputElement.val(filePath);
            valueAccessor(inputElement.val());
            // valueAccessor($("#choose1").find("option:selected").val()+inputElement.val());
          if(inputElement.attr("id")=="val1")
            filePath=choose2_option_val.split(",")[1] + filePath
            inputElement.val(filePath);
            valueAccessor(inputElement.val());
         /*  if(inputElement.attr("id")=="attachment_input_id"){
              filePath=hdfs_curr_lication + filePath
            inputElement.val(filePath);
            valueAccessor(inputElement.val());
            }*/
        }
    ...
    if(typeof inputElement.attr("id")!=undefined && inputElement.attr("id")=="distcp_input"){
       _btn.addClass("disabled").attr("disabled", "disabled");
    }
    return _btn;
  }
  ...
  ko.bindingHandlers.hueach = {
    ...
    update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
      ...
      if (data.length > disableHueEachRowCount) {
        ...
        $parent.height(data.length * itemHeight+10);
