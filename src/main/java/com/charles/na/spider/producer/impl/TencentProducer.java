 ...
 * @author huqj
 */
@Service("tencentProducer")
@Log4j
public class TencentProducer implements ProducerSpider {

    private String rootUrl = "http://new.qq.com";
    ...
    /**
     * 腾讯新闻从链接判断日期的格式是: https://new.qq.com/omn/20181127/20181127A03GZW.html
     */
    ...
    public void init() {
        ...
        new Timer().schedule(new TimerTask() {
            @Override
            public void run() {
                log.info("[tencent] toVisitUrlList length: " + toVisitUrlList.size()
    ...
    public void produce(PriorityQueue queue) {
        ...
        while (pushedArticleNumZk.get() < MAX_ARTICLE_NUM_ONCE && !toVisitUrlList.isEmpty()) {
            ...
            try {
                ...
                log.error("[tencent] error when produce url from: " + url, e);
            }
        }
        //队列空了导致没有新闻继续抓取，打个warning
        if (toVisitUrlList.isEmpty()) {
            log.warn("[tencent] toVisitUrlList is empty! Producing stopped!");
        }
    }

    /**
     * 判断该链接是否是腾讯新闻当日的最新链接
     * 示例： https://new.qq.com/omn/20181127/20181127A03GZW.html
     *
     * @param url
     * @return
     */
    private boolean isFinalNewsPage(@NotNull String url) {
        ...
        if (!url.contains("new.qq.com/omn" + dateStr)) {
            return false;
        }
    ...
    private List<String> extractSinaUrl(String url) {
        ...
        while (start != -1) {
            ...
            if (end != -1) {
                ...
                if ((link.startsWith("http://new.qq.com") || link.startsWith("https://new.qq.com"))
    ...
    private boolean isOldNewsLink(String link) {
        ...
        int index = link.indexOf("new.qq.com/omn/");
        if (index != -1) {
            index += 15;
        } else {
            return false;
        ...
        try {
            ...
            log.error("[tencent] error parse date.", e);
        }
        return false;
    }
    ...
    public void setDate(String date) {
        try {
            ...
            log.error("[tencent] error parse exact date.", e);
        }
    }
